...
const QuizForm = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:3000/quiz/create-quiz', { quizQuestion, option1, option2, option3, option4, answer })
      if (res.data.success) {
        navigate("/dashboard");
      }
    } catch (error: any) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-r from-blue-100 to-purple-200">
      <div className="bg-white p-8 rounded-xl shadow-xl w-[80vw]  sm:w-[400px] md:w-[500px]">
